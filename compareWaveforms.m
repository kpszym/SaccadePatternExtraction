...
% FUNCTION: Computes the similarity between each of the
%           saccade waveforms. Does so by comparing each waveform to all
%           the other waveforms and all their possible cyclic shifts.
%           
% INPUT: 
%           waveform_mat - Nx600 matrix containing the saccade waveform 
%           representation of a saccade pattern. 
%
%           shifted_data - Matrix containing all circular shifts of saccade
%           waveforms in waveform_mat.
% 
% OUTPUT:   
%           waveform_distance_matrix - a pxp matrix where p is the amount of
%           patterns being compared. Each element represents the similarity
%           between two specific patterns. 
%
        ...
        if length(examine_vector(start_idx+1:end)) >= size(waveform_mat,1)*.05 && size(waveform_mat,1) >= 1000
            ...
            for e = 1:length(binned_comp_vect_idxs)-1 
                ...
                if ~isempty(non_identical_idx)
                    ...
                    % Find the minimum distane from the finer scale
                    % circular shift comparisons
                    waveform_distance_matrix(n, non_identical_idx, 1) = min([comparison_matrix_p1; comparison_matrix_p2]);
                end
                % If totally identical waveforms were found in the current
                % bin, add their idices to the list so that they are not
                % included in future bins
                bin_total_identical_idx = [bin_total_identical_idx, identical_idx];
            end
            for i_idx = bin_total_identical_idx
                examine_vector(find(examine_vector == i_idx)) = []; % Remove indicies marked as identical from future bins
                waveform_distance_matrix(i_idx, (i_idx+1):end, 1) = waveform_distance_matrix(n, (i_idx+1):end, 1); % Carry over the distance matrix values of one waveform to all of its identical waveforms
